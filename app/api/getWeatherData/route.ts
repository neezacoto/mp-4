...
export async function GET(request: Request): Promise<NextResponse>{
    ...
    // If no 'city' parameter is provided, return a 400 Omegalol Request error response
    if(!city){
        return NextResponse.json({error: "No [city] provided"}, {status: 400});
    }

    //Make an API request to visual crossing to fetch weather data for the specificed city
